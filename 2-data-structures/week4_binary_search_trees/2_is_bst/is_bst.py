...
def IsBinarySearchTreeAux(tree, mini, maxi, node):
  ...
  if tree[node][1] == -1:
    return IsBinarySearchTreeAux(tree, tree[node][0] + 1, maxi, tree[node][2])
  
  if tree[node][2] == -1:
    return IsBinarySearchTreeAux(tree, mini, tree[node][0] - 1, tree[node][1])
  
  return IsBinarySearchTreeAux(tree, mini, tree[node][0] - 1, tree[node][1]) and IsBinarySearchTreeAux(tree, tree[node][0] + 1, maxi, tree[node][2])
